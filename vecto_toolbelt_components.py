...
class VectoIngestImage(Component):
    """A component that accepts a str or list of image paths and their metadata, formats it 
    ...
    metadata_list: InCompArg[str] # Union[str, list]
    ...
    def execute(self, ctx) -> None:
        ...
        self.ingestResponse.value = vecto_toolbelt.ingest_image(vecto_client, self.batch_path_list.value, self.metadata_list.value)
...
class VectoIngestAllImages(Component):
    """A component that accepts a list of image paths and their metadata, then send them
    ...
    def execute(self, ctx) -> None:
        ...
        vecto_client = self.vecto_client.value if self.vecto_client.value else ctx['vecto_client']
        self.ingestResponse.value = vecto_toolbelt.ingest_all_images(vecto_client, self.path_list.value, self.metadata_list.value, self.batch_size.value)

        self.done = True

@xai_component
class VectoIngestText(Component):
    """A component that accepts a str or list of text and their metadata, formats it 
    in a list of dicts to be accepted by the ingest function. 
    ...
    def execute(self, ctx) -> None:
        ...
        vecto_client = self.vecto_client.value if self.vecto_client.value else ctx['vecto_client']
        self.IngestResponse.value = vecto_toolbelt.ingest_text(vecto_client, self.batch_text_list.value, self.metadata_list.value)

        self.done = True

@xai_component
class VectoIngestAllText(Component):
    """A component that accepts a list of text and their metadata, then send them
    ...
    def execute(self, ctx) -> None:
        ...
        vecto_client = self.vecto_client.value if self.vecto_client.value else ctx['vecto_client']
        self.ingestResponse.value = vecto_toolbelt.ingest_all_text(vecto_client, self.path_list.value, self.metadata_list.value, self.batch_size.value)
